 ...
 * 基础model
 */
class MeetingRecordModel extends Model{
     ...
     * 获取行数数据
     */
    public function getCountBySql($sql,$alias){
      //  print_r($sql);die;
        $result=Db::query($sql);
        return $result[0][$alias]>0?$result[0][$alias]:0;
    }
     ...
     * @return array            分页数据
     */
    public function getPageBySql($sql,$limit=10,$count,$params){
       // print_r($params);
        if($count>0){
            $page=new_page($count,$limit,$params);
            $list=Db::query($sql.' limit '.$page->firstRow.','.$page->listRows);
            $data=array(
                'data'=>$list,
                'page'=>$page->show()
            );
        }else {
            $page=new_page(0,10,$params);
            $data=array(
                'data'=>array(),
                'page'=>$page->show()
            );
