...
class SignrecordController extends AdminBaseController{
    ...
    public function ajaxSignrecordList(){
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $rows = isset($_GET['rows']) ? intval($_GET['rows']) : 10;
        $name=I("get.name");
        $memberName=I("get.memberName");
        $offset = ($page-1)*$rows;
        $countsql="select count(s.id) AS total from qfant_signrecord s ,qfant_partybranch p,qfant_member m WHERE s.partybranch_id = p.id AND s.member_id = m.id";
        $sql="select p.name,m.truename,s.* from qfant_signrecord s ,qfant_partybranch p,qfant_member m WHERE s.partybranch_id = p.id AND s.member_id = m.id";
        $param=array();
        if(!empty($name)){
        ...
        if(!empty($memberName)){
            array_push($param,'%'.$memberName.'%');
            $countsql.=" and m.truename like '%s' ";
            $sql.=" and m.truename like '%s' ";
        }
        $sql.=" order by s.signtime desc  limit %d,%d";
        array_push($param,$offset);
        array_push($param,$rows);
        $data=D('Signrecord')->query($countsql,$param);
        ...
        $data=D('Signrecord')->query($sql,$param);
        foreach ($data as $key=>$basevalue) {
            $data[$key]['signtime'] = date('Y-m-d H:i:s',$basevalue['signtime']);
        }
        $result["rows"] = $data;
        $result["status"] = 1;
