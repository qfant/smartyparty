...
class PartyManagerController extends AdminBaseController{
    public function index(){
        $this->display();
    }

    public function ajaxPartyManagerList(){
        ...
        $name=I("get.name");
        $offset = ($page-1)*$rows;
        $countsql="select count(n.id) AS total from  qfant_partymanager n where 1=1 AND n.department_id in (".$childs.")";
        $sql="select n.* from qfant_partymanager n  where 1=1 AND n.department_id in (".$childs.")";
        $param=array();
        if(!empty($name)){
        ...
        $data=D('Partymanager')->query($countsql,$param);
        ...
        $data=D('Partymanager')->query($sql,$param);
